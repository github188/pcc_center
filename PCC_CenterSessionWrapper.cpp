...
static INT_PTR InitializeAllCallsTypeInfo_()
{
...

	{
		BYTE const chZipped[] =
		{
			,
		};
		static CBinary idTxt;
		CBinary& bin = idTxt;
		VERIFY(QuickUnzip(chZipped, sizeof(chZipped), bin));
...

	{
		BYTE const chZipped[] =
		{
			0x51,0x00,0x00,0x80,0x78,0x01,0x0B,0x70,0x76,0x8E,0x0F,0x2D,0x4E,0x2D,0xB2,0xB2,
			0x72,0xA9,0xCC,0x4B,0xCC,0xCD,0x4C,0x76,0xCE,0xCF,0x2B,0x49,0xAD,0x28,0x51,0xF1,
			0xF4,0x0B,0x31,0x36,0xD2,0x41,0x26,0xAD,0x73,0x53,0x4B,0x32,0xF2,0x53,0x6A,0x3C,
			0xFD,0x6A,0x8A,0x4B,0x8A,0x32,0xF3,0xD2,0x75,0xFC,0x43,0x43,0x6A,0x50,0xB5,0xE9,
			0x30,0x00,0x00,0x64,0x89,0x1B,0xE3,
		};
		static CBinary idTxt;
		CBinary& bin = idTxt;
		VERIFY(QuickUnzip(chZipped, sizeof(chZipped), bin));
...

	{
		BYTE const chZipped[] =
		{
			0x4E,0x00,0x00,0x80,0x78,0x01,0x0B,0x70,0x76,0x8E,0x0F,0x2D,0x4E,0x2D,0xB2,0xB2,
			0x72,0xAD,0x48,0x4D,0x2E,0x2D,0x49,0x0D,0x2E,0x49,0x2C,0x49,0x55,0xF1,0xF4,0x0B,
			0x31,0x36,0xD2,0xB1,0xCE,0x4D,0x2D,0xC9,0xC8,0x4F,0xA9,0xF1,0xF4,0xAB,0x49,0x2C,
			0x2A,0x4A,0xAC,0xB4,0x01,0x0A,0x9B,0x99,0xD8,0xE9,0xF8,0x87,0x86,0x40,0x05,0x90,
			0x35,0xD9,0xE9,0x30,0x00,0x00,0x35,0x15,0x1B,0x2F,
		};
		static CBinary idTxt;
		CBinary& bin = idTxt;
		VERIFY(QuickUnzip(chZipped, sizeof(chZipped), bin));
				...
				) callback
{
	...
	{
		const PCC_Center_T::PCC_ModelFile& ref1 = modelFiles_wrap[idx1];
				...
				) method
{
	...
	{
		iscm_RPCReplyPrefix replyPrefix_iscm;
		OUT tcps_Array<PCC_User_T::NodeDesc> nodes;
		OutParamWrapSite() { replyPrefix_iscm.Init(); }
		~OutParamWrapSite() { }
		static void FreeAction(void* p)
		{
	...
	OUT const tcps_Array<PCC_User_T::NodeDesc>& nodes_wrap = opWrapper->nodes;
	Set_BaseType_(outfiter, nodes_wrap.LenRef());
	for(int idx1=0; idx1<nodes_wrap.Length(); ++idx1)
	{
		const PCC_User_T::NodeDesc& ref1 = nodes_wrap[idx1];
				...
				) method
{
	...
	{
		iscm_RPCReplyPrefix replyPrefix_iscm;
		OUT PCC_User_T::DynamicContext dynamicContext;
		OutParamWrapSite() { replyPrefix_iscm.Init(); }
		~OutParamWrapSite() { }
		static void FreeAction(void* p)
		{
	...
	// OUT DynamicContext dynamicContext
	OUT const PCC_User_T::DynamicContext& dynamicContext_wrap = opWrapper->dynamicContext;
				...
				) method
{
	...
	{
		iscm_RPCReplyPrefix replyPrefix_iscm;
		OUT tcps_Array<PCC_User_T::ExecuteState> jobsState;
		OutParamWrapSite() { replyPrefix_iscm.Init(); }
		~OutParamWrapSite() { }
		static void FreeAction(void* p)
		{
	...
	OUT const tcps_Array<PCC_User_T::ExecuteState>& jobsState_wrap = opWrapper->jobsState;
